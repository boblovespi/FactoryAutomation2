...
public class BrickCrucibleBER implements BlockEntityRenderer<BrickCrucibleBE>
{
	private final BakedQuad quad;

	public BrickCrucibleBER(BlockEntityRendererProvider.Context pContext)
	{
		var lower = 3 / 16f;
		var upper = 13 / 16f;
	...
	@Override
	public void render(BrickCrucibleBE be, float d, PoseStack stack, MultiBufferSource buffer, int packedLight, int packedOverlay)
	{
		if (be.getAmount() > 0)
		{
			stack.pushPose();
			{
				...
				stack.translate(0, 1/16f + 0.82 * be.getAmount(), 0);
