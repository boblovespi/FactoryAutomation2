...
public class TripHammerBER extends GeoBlockRenderer<TripHammerBE> {
    ...
    public boolean shouldRender(TripHammerBE blockEntity, Vec3 cameraPos) {
        if(blockEntity.getLevel().getBlockState(blockEntity.getBlockPos()).is(FABlocks.TRIP_HAMMER.get()))
            return blockEntity.getLevel().getBlockState(blockEntity.getBlockPos()).getValue(TripHammer.MULTIBLOCK_COMPLETE);
        return false;
    }
    ...
    @Override
    public boolean shouldRenderOffScreen(TripHammerBE blockEntity) {
        return true;
    }

    @Override
    public AABB getRenderBoundingBox(TripHammerBE blockEntity) {
        return new AABB(blockEntity.getBlockPos().above(3).east(5).north(5).getCenter(), blockEntity.getBlockPos().west(5).south(5).getCenter());
    }
}
